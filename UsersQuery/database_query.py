...
messages_query = (f"SELECT Date, Amount FROM Messages WHERE PersonId = ? AND Date BETWEEN '{start_date}' AND '{end_date}'")

tasks_query_1 = ("SELECT DISTINCT PersonId FROM TasksChanges")
tasks_query_2 = (f"SELECT Date, Amount FROM TasksChanges WHERE PersonId = ? AND Date BETWEEN '{start_date}' AND '{end_date}'")